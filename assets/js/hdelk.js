...
var hdelk = (function(){
   ...
   // var edge_bus_highlight_color = ['#666', '#4bf', '#F66', '#ED0', '#7e0']; // light edges
    ...
    var layout = function( graph, divname  ) {
        ...
        var transformed = JSON.stringify(graph, null, " ");

        elk.layout(graph)
            .then(function(g) {
                var dp = document.getElementById( divname + "_preprocessed" );
                if ( dp )
                    dp.innerHTML = "<pre style='font-size:10px'>" + transformed + "</pre>";
    ...
    var transformNode = function( child ) {
        ...
        // var labelCandidate = ( child.label || child.label == "" ) ? child.label : child.id;
        // var labelItem = { text:labelCandidate, width:labelCandidate.length * ( 3 * node_port_height / 4 ) / 2, height:node_label_height };
        // var typeCandidate = ( child.label || child.label == "" ) ? child.label : child.id;
        // var typeItem = { text:labelCandidate, width:labelCandidate.length * ( 3 * node_port_height / 4 ) / 2, height:node_label_height };
        // child.labels = [ labelItem, typeItem ];
